...
export async function getNextInvoiceNumber(accountId: string) {
...
export async function listInvoices(accountId: string, limit = 100) {
  const query: any = {};
  if (accountId) {
    try {
 ...
 */
export async function createInvoiceFromOrder(order: any, createdBy: string) {
  ...
  const subtotal = items.reduce(
    (s: number, it: any) =>
      s + parseFloat((it.quantity * it.unitPrice).toFixed(2)),
    0
  );
  ...
  const taxTotal = items.reduce(
    (s: number, it: any) =>
      s +
      parseFloat(
        ((it.quantity * it.unitPrice * (it.taxPercent || 0)) / 100).toFixed(2)
      ),
    0
  );
  ...
  const invoiceNumber = await getNextInvoiceNumber(
    String(order.accountId ?? (order.accountId || "1"))
  );
...
export async function findInvoiceByGuestToken(token: string) {
