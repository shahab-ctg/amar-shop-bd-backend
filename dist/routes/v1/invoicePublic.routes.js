...
router.get("/invoices/by-order/:orderId", async (req, res) => {
    try {
        ...
        const tryObjectIdLookup = async (val) => {
            try {
                ...
                console.warn("objectId lookup error for", val, e && e.message ? e.message : e);
            }
            return null;
        };
        ...
        if (byObj)
            return res.json({ ok: true, data: byObj });
        // 2) numeric fallback
        ...
        if (!Number.isNaN(maybeNum)) {
            try {
                ...
                if (invByNum)
                    return res.json({ ok: true, data: invByNum });
            }
            catch (e) {
                console.warn("numeric lookup error for", orderId, e && e.message ? e.message : e);
            }
        }
        ...
        try {
            ...
            if (invByString)
                return res.json({ ok: true, data: invByString });
        }
        catch (e) {
            console.warn("string lookup error for", orderId, e && e.message ? e.message : e);
        }
        // Not found
        return res.status(404).json({ ok: false, error: "not found" });
    }
    catch (err) {
        console.error("public invoice by-order err:", err && err.stack ? err.stack : err);
